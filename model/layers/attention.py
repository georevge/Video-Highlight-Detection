...
class SelfAttention(nn.Module):
    ...
    def getAbsolutePosition(self, T):
        ...
        :param int T: Number of frames contained in Q
    ...
    def forward(self, x):
        ...
        for head in range(self.heads):
            ...
            y = torch.matmul(att_weights, V)  # torch.matmul(_att_weights, V)

            # Save the current head output
            outputs.append(y)
            # att_weights_list.append(att_weights)
        y = self.out(torch.cat(outputs, dim=1))
        # att_weights = torch.stack(att_weights_list)
        # att_weights = torch.mean(att_weights, dim=0)
        # y_sum = torch.matmul(att_weights, y)  # for 1 head
        y = self.drop(y)
