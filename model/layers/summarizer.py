...
from layers.second_attention import Attention
...
class MultiAttention(nn.Module):
        ...
        if self.num_segments is not None:
            ...
            for _ in range(self.num_segments):
                ...
                self.local_attention.append(SelfAttention(input_size=input_size, output_size=output_size//num_segments,
                                                          freq=freq, pos_enc=pos_enc, heads=8))
    ...
    def forward(self, frame_features):
        ...
        """
        weighted_value, attn_weights = self.attention(frame_features)  # global attention

        ##################
        # check this part
        if self.num_segments is not None and self.fusion is not None:
            segment_size = math.ceil(frame_features.shape[0] / self.num_segments)
            for segment in range(self.num_segments):
                left_pos = segment * segment_size
                right_pos = (segment + 1) * segment_size
                local_x = frame_features[left_pos:right_pos]
...
class PGL_SUM(nn.Module):
    def __init__(self, input_size=1024, output_size=1024, freq=10000, pos_enc=None,
        ...
        self.second_attention = Attention(input_size=input_size, output_size=output_size)
        self.second_attention_reconstr = Attention(input_size=input_size, output_size=output_size, cap_size=input_size)
        ...
        self.linear_layer = nn.Linear(in_features=input_size, out_features=output_size, bias=True)

    def forward(self, shot_features):
        """ Produce frames importance scores from the frame features, using the PGL-SUM model.

        :param torch.Tensor shot_features: Tensor of shape [T, input_size] containing the frame features produced by
        using the pool5 layer of GoogleNet.
        :return: A tuple of:
            y: Tensor with shape [1, T] containing the frames importance scores in [0, 1].
            attn_weights: Tensor with shape [T, T] containing the attention weights.
        """
        residual = shot_features
        weighted_value, attn_weights = self.attention(shot_features)
        y = weighted_value + residual
        y_norm = self.norm_y(y)
        y_ff = self.linear_layer(y_norm)
        zeros = torch.zeros(len(y_ff), 512).to("cuda")
        y_ff = torch.maximum(y_ff, zeros)
        ...
        zeros = torch.zeros(len(video_embedding_ff), 512).to("cuda")
        video_embedding_ff = torch.maximum(video_embedding_ff, zeros)
        video_embedding_ff2 = self.linear_layer(video_embedding_ff)
